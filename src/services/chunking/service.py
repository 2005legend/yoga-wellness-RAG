...
from src.models.schemas import Chunk, ContentCategory
...
class ChunkingService(LoggerMixin):
    ...
    def _validate_chunks(self, chunks: List[Chunk]) -> List[Chunk]:
        ...
        for chunk in chunks:
            ...
            # Check token count
            if chunk.metadata.tokens < self.config.min_chunk_size:
                self.logger.debug(f"Skipping chunk {chunk.id}: below minimum tokens")
                continue
